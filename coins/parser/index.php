...
for($i = 0; $i<count($storedDates); $i++){
    if(preg_match('/ICO/', $storedDates[$i]['name']) and !preg_match('/no ICO/', $storedDates[$i]['name'])) {
        $storedDates[$i]['name'] = preg_replace('/	/', '', $storedDates[$i]['name']);
        $bool = $db->twitter($storedDates[$i]);
        if ($bool) {print "<br> twitting";
        print_r($storedDates[$i]['name']);
        }
    }
...
function takeDate($linksArray, $container)
    { 
        foreach($linksArray as $link){
            print "<br> serving ".$link;
            $ind = count($container);
            $opts    = array('http'=>array('method'=>"GET",'header'=>"Accept-language: en\r\n" ."User-Agent: not for you\r\n"));
            $context = stream_context_create($opts);
            $html    = new simple_html_dom();
            $html    = file_get_html($link, false, $context);
            if($html == false || !isset($html)) continue;
